...
dojo.declare("bespin.client.session.EditSession", null, {
        ...
        bespin.fireAfter([ "settings:loaded" ], function() {
            bespin.subscribe("settings:set:collaborate", function(ev) {
                ...
                if (bespin.get("settings").isOn(ev.value)) {
                    // Collaboration is on, turn it off
    ...
    startSession: function(project, path, onSuccess, onFailure) {
        ...
        this.editor.moveCursor({ row: 0, col: 0 });
	...
	if (! collabList) { return }
        ...
        // Create user object from each userEntry string, which collates the
        // windows for that user
        var users = {};
        var userCount = 0;
        userEntries.forEach(function(userEntry) {
            // userEntry is of the form "username:ip-addr:window-id"
            ...
            user.windows.push({ address: parts[1], id: parts[2] });
        });
        ...
        for (var username in users) {
            if (users.hasOwnProperty(username)) {
                var user = users[username];
                var extra = "";
                if (user.windows.length > 1) {
                    extra += " <small>(" + user.windows.length + ")</small>";
                }

                var compact = userCount > maxInLargeMode;

                var title = user.windows.length + " window" +
                        (user.windows.length > 1 ? "s" : "");
                user.windows.forEach(function(window) {
...
dojo.declare("bespin.client.session.ShareNode", null, {
    ...
    raiseError: function(text, recoverable) {
        if (!text) {
            text = "(No server error message)";
        }
        var prefix = "<strong>" + (recoverable ? "" : "Fatal ") + "Collaboration Error</strong>: ";
        var suffix = "<br/><strong>Warning</strong>: Changes to this file could be lost";

        var commandLine = bespin.get("commandLine");
        if (commandLine) {
            commandLine.showHint(prefix + text + suffix);
        } else {
    ...
    getMetaData: function() {
        var data = {
            cursor: this.captureSimpleCursor()
        };
    ...
    captureSimpleCursor: function() {
        ...
        return {
            startOffset: this.convertRowColToOffset(start),
            endOffset: this.convertRowColToOffset(end)
        };
    },
