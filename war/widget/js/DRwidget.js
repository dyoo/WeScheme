var initializeWidget = (function () {
   ...
   function getDRCode(){
        ...
        for(var i=0; i<examples.length; i++)
            exampleText += "(EXAMPLE "+ values.examples[i].header + ws('example1_wrapper') + values.examples[i].body  + ")\n"; 
        }

        var contract= "; "+values.name+" : "+values.domain+" -> "+values.range;
        var example1     = "(EXAMPLE "+ values.examples[1].header + ws('example1_wrapper') + values.examples[1].body  + ")";   
        var example2     = "(EXAMPLE "+ values.examples[1].header + ws('example2_wrapper') + values.examples[1].body  + ")";   
        var definition   = "(define  "+ values.def_header + ws('definition_wrapper')+values.def_header+ ")";
   ...
   clearForm = function (){
        ...
        values.examples[1].errorDOM.innerHTML = "";
        values.examples[2].errorDOM.innerHTML = "";
        values.definition_error.innerHTML = "";
